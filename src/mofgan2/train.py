...
def get_arg_parser():
    ...
    parser.add_argument(
        ...
        # default=[32, 32, 64, 128, 256, 512],  # CURRENT
        # default=[8, 8, 16, 32, 64, 128, 128, 128, 128],
        # default=[4, 4, 8, 16, 32, 64, 64, 64, 64],
        # default=[8, 8, 16, 32, 64, 128],
        metavar='CHANNELS'
    )
...
def get_dataset(args):
    ...
    # return MOFDataset.load(args.data_dir)
    # result = TestDataset.load(args.data_dir)
    # print("BOUNDS:", result.data.min(), result.data.max())
    # return result

    assert args.data_dir, '--data_dir has to be specified.'
    # height, width = [
    #     shape * 2 ** (len(args.d_channels or args.channels) - 1) for shape in args.base_shape
    # ]
...
def get_trainer(args):
    ...
    if args.resume and stylegan2.trainer._find_checkpoint(args.checkpoint_dir):
        trainer = stylegan2.trainer.Trainer.load_checkpoint(
            args.checkpoint_dir,
            dataset,
            device=args.gpu,
            rank=args.rank,
        ...
        G, D = get_models(args)
        trainer = stylegan2.trainer.Trainer(
            G=G,
            D=D,
            latent_size=args.latent,
            dataset=dataset,
