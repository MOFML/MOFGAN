        ...
        for iteration in range(iterations):
            ...
            for _ in range(self.G_iter):
                ...
                for i in range(self.subdivisions):
                    ...
                    # print(latents.shape, latent_labels)
                    loss, _ = self.G_loss(
                        G=self.G,
                        D=self.D,
                        latents=latents,
            ...
            if iteration % 100 == 0:
                images = self.generate_images(num_images=32)
                # pil_to_tensor = torchvision.transforms.PILToTensor()
                # print(images[0])
                # print(images[0].convert('RGB'))
                # images: List[Tensor] = [pil_to_tensor(x) for x in images]
                # print(images[0])
                # print(images[0].shape)
                torchvision.utils.save_image(
                    images,
                    f"sample/{str(iteration).zfill(6)}.png",
                    # nrow=int(32 ** 0.5),
                    # normalize=True,
                    # range=(-1, 1),
                )
