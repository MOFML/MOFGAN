...
def prepare(env, dataset, n_worker, sizes=(128, 256, 512, 1024), resample=Image.LANCZOS):
    ...
    with multiprocessing.Pool(n_worker) as pool:
        for i, imgs in tqdm(pool.imap_unordered(resize_fn, files), ncols=80, unit='images', total=len(files)):
...
def main():
    ...
    image_folder = datasets.ImageFolder(args.path)

    if not args.out:
        print("Output folder missing!")

    with lmdb.open(args.out, map_size=1024 ** 4, readahead=False) as env:
        prepare(env, image_folder, args.n_worker, sizes=sizes, resample=resample)
